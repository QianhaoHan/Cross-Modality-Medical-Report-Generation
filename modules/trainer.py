...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        if 1:
            for batch_idx, (images_id, images, reports_ids, reports_masks , retrival_ids , retrival_mask) in enumerate(self.train_dataloader):

                images, reports_ids, reports_masks ,retrival_ids ,retrival_mask = images.to(self.device), reports_ids.to(self.device), \
                                                    reports_masks.to(self.device) , retrival_ids.to(self.device) , retrival_mask.to(self.device)
                #print(reports_ids)
                output = self.model(images, reports_ids, retrival_ids,mode='train')
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            if 1:
                for batch_idx, (images_id, images, reports_ids, reports_masks, retrival_ids , retrival_mask) in enumerate(self.val_dataloader):
                    # print(retrival_ids)
                    # print(images)
                    images, reports_ids, reports_masks,retrival_ids , retrival_mask = images.to(self.device), reports_ids.to(
                        self.device), reports_masks.to(self.device),retrival_ids.to(self.device) , retrival_mask.to(self.device)

                    output, _ = self.model(images,retrival_ids = retrival_ids, mode='sample')
        ...
        with torch.no_grad():
            test_gts, test_res = [], []
            if 1:
                for batch_idx, (images_id, images, reports_ids, reports_masks,retrival_ids , retrival_mask) in enumerate(self.test_dataloader):
                    images, reports_ids, reports_masks ,retrival_ids , retrival_mask = images.to(self.device), reports_ids.to(
                        self.device), reports_masks.to(self.device), retrival_ids.to(self.device) , retrival_mask.to(self.device)
                    output, _ = self.model(images,retrival_ids = retrival_ids, mode='sample')
                    reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
